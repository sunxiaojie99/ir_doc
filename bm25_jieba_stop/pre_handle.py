...
def pre_handle_query(query_file, jieba_query, stop_word_file=None):
    if stop_word_file is not None:
        stopwords = [line.strip() for line in open(stop_word_file,encoding='UTF-8').readlines()]  # 1206个
...
# 1. 处理query
stop_word_file = os.path.join(here, 'stopwords.txt')
dev_query = os.path.join(here, '../bm25/dev_queries_zh.tsv')
jieba_query = os.path.join(here, 'dev_queries_zh.tsv')
# pre_handle_query(dev_query, jieba_query, stop_word_file=stop_word_file)
...
pre_handle_passage(passage_file, jieba_file, stop_word_file)